...
main(argc,argv)
        ...
        char *argv[];
{
	int i, k, howmany, segsites, largestnsam=0;
	char **list, **cmatrix(), **tbsparamstrs ;
	FILE *pf, *fopen() ;
	double probss, tmrca, ttot ;
	void seedit( const char * ) ;
	void getpars( int argc, char *argv[], int *howmany )  ;
	int gensam( char **list, double *probss, double *ptmrca, double *pttot, int largestnsam ) ;
    ...
    while( howmany-count++ ) {
      ...
      fprintf(pf,"\n//");
	   if( ntbs >0 ){
		...
		/* Added by JRI Aug 2007.  Re-allocates memory to list if new sample size is bigger than old */
		if( pars.cp.nsam>largestnsam ){
 		...
 		segsites = gensam( list, &probss, &tmrca, &ttot, largestnsam ) ; 
...
int gensam( char **list, double *pprobss, double *ptmrca, double *pttot, int largestnsam ) 
{
  	...
  	if( (segsitesin == 0) && ( theta > 0.0)   ) {
		...
		for( seg=0, k=0; k<nsegs; seg=seglst[seg].next, k++) { 	  	
			...
			if( (segsit + ns) >= maxsites ) {
				...
				posit = (double *)realloc(posit, (unsigned) maxsites*sizeof(double) ) ;
...
getpars(int argc, char *argv[], int *phowmany )
{
  ...
  if( count == 0 ) {
	...
	*phowmany = atoi( argv[2] );
	...
	/* changed from double * to double by JRI Aug. 2007 */
